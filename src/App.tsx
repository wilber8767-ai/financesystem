  ...
  return (
    <div style={{ position: 'relative', display: 'flex', alignItems: 'center', minWidth: 0 }}>
      ...
      <input type="number" value={value} placeholder={placeholder || '0'}
        ...
        style={Object.assign({}, bigBase, { paddingLeft: pre ? 34 : 16, paddingRight: suf ? 54 : 16, borderColor: hl ? C.ros : (f ? C.ind : C.s200), boxShadow: hl ? '0 0 0 3px rgba(225,29,72,0.18)' : f ? '0 0 0 4px rgba(79,70,229,0.18)' : 'none' })} />
...
function TI({ value, onChange, placeholder, type }: { value: string; onChange: (v: string) => void; placeholder?: string; type?: string; }) {
  ...
  return (
    <input type={type || 'text'} value={value} placeholder={placeholder || ''}
      onChange={function(e) { onChange(e.target.value); }}
      onFocus={function() { sf(true); }} onBlur={function() { sf(false); }}
      style={Object.assign({}, txtBase, { borderColor: f ? C.ind : C.s200, boxShadow: f ? '0 0 0 4px rgba(79,70,229,0.18)' : 'none' })} />
  );
}
...
function SI({ value, onChange, opts }: { value: string; onChange: (v: string) => void; opts: { value: string; label: string }[]; }) {
  ...
  return (
    <div style={{ position: 'relative', minWidth: 0, width: '100%' }}>
      <select value={value} onChange={function(e) { onChange(e.target.value); }} onFocus={function() { sf(true); }} onBlur={function() { sf(false); }}
        style={Object.assign({}, txtBase, { paddingRight: 38, cursor: 'pointer', borderColor: f ? C.ind : C.s200, boxShadow: f ? '0 0 0 4px rgba(79,70,229,0.18)' : 'none' })}>
...
function PC({ gradient, icon, title, sub, children }: { gradient: string; icon: string; title: string; sub: string; children: React.ReactNode; }) {
  return (
    <div style={Object.assign({}, cardSt)}>
...
function Report({ client, prot, onBack }: { client: Client; prot: Prot; onBack: () => void; }) {
  ...
  var cashflow = useMemo(function() {
    ...
    for (var yr = age; yr <= 90; yr++) {
      ...
      if (yr < retAge) { asset = asset * (1 + GROW) + Math.max(realMonthlySave, 0) * 12; }
      else { asset = asset * (1 + GROW * 0.4) - inflExp; }
    }
    return rows;
          ...
          <div className="sp">
            ...
            <div className="gfm" style={{ marginBottom: 16 }}>
              ...
              <div style={{ background: 'rgba(225,29,72,0.15)', border: '2px solid rgba(225,29,72,0.45)', borderRadius: 14, padding: 16 }}>
                ...
                <div style={{ color: '#fca5a5', fontSize: 11, marginTop: 6 }}>購買力縮水 {((retMonthExp / Math.max(expense, 1) - 1) * 100).toFixed(0)}%</div>
              </div>
              <div style={{ background: 'rgba(217,119,6,0.15)', border: '2px solid rgba(217,119,6,0.4)', borderRadius: 14, padding: 16 }}>
                ...
                <div style={{ background: 'rgba(0,0,0,0.22)', borderRadius: 8, padding: '6px 10px', marginTop: 8 }}>
                  <div style={{ color: '#fcd34d', fontSize: 11 }}>{D(retMonthExp)} x 12月 x {retYears}年（85-{retAge}歲）</div>
                </div>
            ...
            <div style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid ' + C.s700, borderRadius: 14, padding: 16 }}>
              <div style={{ color: '#e2e8f0', fontWeight: 900, fontSize: 15, marginBottom: 14, textAlign: 'center' }}>💡 為了達成目標，您現在需要做的是...</div>
              <div className="gac">
                ...
                <div style={{ background: retGap > 0 ? 'rgba(225,29,72,0.12)' : 'rgba(52,211,153,0.1)', border: '2px solid ' + (retGap > 0 ? 'rgba(225,29,72,0.38)' : 'rgba(52,211,153,0.3)'), borderRadius: 12, padding: 14 }}>
                  ...
                  <div className="num-xl" style={{ color: retGap > 0 ? '#fb7185' : '#34d399', fontWeight: 900 }}>{retGap > 0 ? DW(annualNeed) : '✅ 已足備'}</div>
                  {retGap > 0 && (
                    <div style={{ background: 'rgba(0,0,0,0.18)', borderRadius: 7, padding: '5px 8px', marginTop: 7 }}>
                ...
                <div style={{ background: retGap > 0 ? 'rgba(225,29,72,0.12)' : 'rgba(52,211,153,0.1)', border: '2px solid ' + (retGap > 0 ? 'rgba(225,29,72,0.38)' : 'rgba(52,211,153,0.3)'), borderRadius: 12, padding: 14 }}>
                  ...
                  <div className="num-xxl" style={{ color: retGap > 0 ? '#fb7185' : '#34d399', fontWeight: 900 }}>{retGap > 0 ? D(monthlyNeed) : '✅ 已足備'}</div>
                  {retGap > 0 && (
                    <div style={{ background: 'rgba(0,0,0,0.18)', borderRadius: 7, padding: '5px 8px', marginTop: 7 }}>
        ...
        <div style={{ background: C.s800, borderRadius: 20, border: '1px solid ' + C.s700 }}>
          <div className="sp">
            <div style={{ marginBottom: 14 }}>
              ...
              <div style={{ color: C.s400, fontSize: 12, marginTop: 3 }}>月儲蓄 {D(Math.max(realMonthlySave, 0))} x 5% 複利，退休後依通膨支出消耗</div>
            </div>
            {depleteAge && (
        ...
        <div style={Object.assign({}, cardSt, { borderLeft: '8px solid ' + C.vio })}>
          <div className="sp">
            ...
            <div className="g2">
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                <div style={gCell}><label style={lblSt}>預計退休年齡</label><SI value={client.retirementAge} onChange={sc('retirementAge')} opts={[55, 58, 60, 62, 65, 67, 70].map(function(v) { return { value: String(v), label: v + ' 歲' }; })} /></div>
                {income > 0 && (
